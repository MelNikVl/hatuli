...
# Было 10 (см. задачу "хайп — 3 месяца вместо 14 дней"): дедуп через
# processed_articles делает расширение окна дешёвым — уже проанализированные
# статьи не переспрашиваются у LLM повторно, окно лишь расширяет пул
# КАНДИДАТОВ (страховка от отставания прогона), не объём реальной LLM-работы
# за прогон (её всё ещё режет MAX_ARTICLES_PER_RUN ниже).
