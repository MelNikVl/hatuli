...
# Найдено при разборе задачи 2026-08-16 ("Локальный OSM-слой",
# scripts/sync_city_poi.py): любой city_poi kind БЕЗ явной записи в
# POI_WEIGHTS получает от _poi_score() ниже ДЕФОЛТНЫЙ вес 5 — это уже
# так для существующих 'shop'/'park'/'clinic'/'gov' (не трогаем,
# устоявшееся поведение). Но sync_city_poi.py добавляет в city_poi НА
# ПОРЯДКИ более плотные категории (road_major/road_secondary —
# сэмплированные ТОЧКИ ВДОЛЬ каждой дороги, ~1 на 150м; railway — то же
# для ж/д) и явно НЕГАТИВНЫЕ категории (industrial/cemetery/landfill/
# bar) — без явного 0 здесь они молча утекли бы в _poi_score() как
# ОБЫЧНЫЕ +5 амбиент-очки, то есть промзона/кладбище/дорога у окна
# ПОВЫШАЛИ бы "инфраструктурную насыщенность" гекса наравне с аптекой,
# а плотные точки дорог попросту забили бы кэп min(100, ...) шумом,
# вытеснив реальный сигнал (магазины/сады) при сортировке кандидатов
# внутри гекса. weight=0 — не участвуют в этом индексе вовсе; у каждой
# уже есть (или появится) собственный сигнал в другом месте: дороги —
# bot/score_layers/noise.py (штраф, не бонус), industrial/cemetery/
# landfill/bar/railway/sports — синхронизируются в city_poi про запас
# (задача просила категории даже без потребителя сейчас), но НЕ должны
# незаметно становиться позитивным весом здесь.
POI_WEIGHTS.update({
    "road_major": 0, "road_secondary": 0, "railway": 0,
    "industrial": 0, "cemetery": 0, "landfill": 0, "bar": 0, "sports": 0,
})
