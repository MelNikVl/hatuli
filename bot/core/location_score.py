...
# _TOTAL_ADJ_MIN/MAX). ЭТО НЕ СЛУЧАЙНО: stability-тест (задача 2026-08-15,
# коммит "Stability-тест", tests/test_location_score_stability.py) нашёл
# честную границу гарантии — добавление НОВОГО ФАКТОРА внутри
# существующей группы строго нейтрально (Δ=0) для ЖК, где он unknown, а
# вот добавление НОВОЙ ГРУППЫ с весом, забранным у существующей, ТАК НЕ
# гарантировано: если группа-донор веса была у своего края (100%/0%) для
# конкретного ЖК, урезание её доли неизбежно сдвигает итог. Поэтому смена
# весов ГРУПП (в отличие от добавления фактора В группу) ВСЕГДА требует
# ручного решения и явного гейта на реальных данных, не тихий рефакторинг.
