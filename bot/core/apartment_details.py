...
async def fetch_apartment_details(url: str) -> dict:
    ...
    # Реальный ключ на странице — "seller":{"name":"Имя Фамилия",...} (даёт
    # ПОЛНОЕ имя с фамилией, не только имя, вопреки более старому
    # предположению ниже). userName/contactName/author.name — не встречались
    # ни на одной проверенной странице, оставлены как фолбэк на случай
    # другой раскладки объявления.
    for pat in (r'"seller"\s*:\s*\{[^}]*?"name"\s*:\s*"([^"\d][^"]{1,50})"',
               r'"userName"\s*:\s*"([^"\d][^"]{1,40})"',
    ...
    # ── Флаг «Срочно, торг» ────────────────────────────────────────────────
    # Активные платные флажки объявления лежат в коротком массиве
    # "services":[{"name":"s"},...] — "s" и есть «Срочно, торг» (не путать
    # с каталогом всех ПОКУПАЕМЫХ услуг в "paidServices", он есть у любого
    # объявления независимо от того, что реально включено).
    svc_m = _re.search(r'"services"\s*:\s*(\[[^\]]{0,300}\])', resp.text)
    if svc_m:
        result["is_urgent"] = '"name":"s"' in svc_m.group(1)
