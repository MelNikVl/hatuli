#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Сбор квартир Svoy Dom (Астана) с сайта svoydom.kz — полный список ЖК."""
...
def scrape_complex(name, page_url):
    ...
    m4 = re.search(r"data-mode=\"([^\"]+)\"|mode['\"]?\s*[:=]\s*['\"]([^'\"]+)", html)
    mode = (m4.group(1) or m4.group(2) or 'N') if m4 else 'N'
    print(f'  {name}: {total_pages} стр, iblock={iblock}, mode={mode}, sessid={bool(sessid)}', flush=True)
...
def main():
    projects = [
        ...
        ('Jar-Jar', 'https://svoydom.kz/lp/astana/jar_jar/'),
