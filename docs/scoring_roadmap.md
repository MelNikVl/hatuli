...
## Часть 9. Продуктовый трек «Локация», Фаза L1 (данные и инфраструктура) — ✅ КОД ЗАВЕРШЁН 2026-08-14, backfill в фоне

Закрывает п.9 «Часть 3. Локация как продуктовая ось» этого файла и §7
[`location_product_design.md`](location_product_design.md#7-очередь-работ)
— Фаза L1 явно НЕ про UI (это Фаза L2), только данные/инфраструктура.
Разрешена во время [«Паузы по ML»](verdict_strategy.md#пауза-по-ml-2026-08-14--2026-09-14).
План (схемы таблиц, кэш, таймеры) показан и подтверждён заказчиком
(5 решений) до реализации — гейт «план → подтверждение → коммиты»,
[`process.md`](process.md).

1. **Миграция 072** ([`migrations/072_location_market_stats.sql`](../migrations/072_location_market_stats.sql))
   — 3 прироста схемы: `complex_location_scores` (append-only, `PRIMARY
   KEY (complex_id, computed_at)`, group-колонки для будущей Фазы C),
   `hex_market_stats` (новая таблица, гексагон-уровень), `complex_stats_
   history` + `avg_dom_days`/`price_drop_share_30d`/`price_drop_share_60d`.
   6 тестов схемы. **РЕШЕНИЕ 1 плана**: НЕ заведена новая `osm_poi_cache`
   — переиспользован уже существующий с 2026 `osm_cache`
   (`bot/score_layers/osm.py`, grid+kind+TTL 60д), новая таблица была бы
   буквальным дублем.
2. **Healthcheck зеркал Overpass** ([`osm_mirrors_healthcheck.py`](../osm_mirrors_healthcheck.py),
   [`bot/score_layers/osm.py::check_mirrors()`](../bot/score_layers/osm.py))
   — проверяет все 4 зеркала независимо (не как `overpass_request()`,
   который перебирает до первого живого), алерт при <2 живых через
   новый [`bot/core/admin_alert.py::notify_admin()`](../bot/core/admin_alert.py)
   — вынесен из `ai_text_analysis.py` при появлении второго вызывающего
   места, не вторая копия. Живой прогон на сервере: 2/4 живы (maps.
   mail.ru, overpass.kumi.systems). Таймер ежедневно 09:05. 9 тестов.
3. **Рыночные агрегаты по гео** — `complex_stats_snapshot.py` расширен
   (не второй писатель) на `avg_dom_days`/`price_drop_share_30d/60d`
   (тренд не хранится, читается на запросе из уже сохранённых снимков);
   новый [`hex_market_stats_snapshot.py`](../hex_market_stats_snapshot.py)
   — плотность предложения по гексагону (Python-группировка, гексагон
   не выразим в SQL), `resolved_house_id` НЕ участвует (агрегация по
   координате объявления, не по имени ЖК). Живой баг найден и
   исправлен: `main()` не вызывал `app_settings.load()` — тихий фолбэк
   на дефолт `HEX_EDGE_M=50` вместо реально настроенных на проде 100.
   Живой прогон: 2559 гексагонов, edge_m=100, 31358 активных объявлений.
   11 новых тестов (4+7 в двух файлах).
4. **`complex_location_score_snapshot.py`** ([bot/core/location_score.py](../bot/core/location_score.py)
   не менялась по сути — только добавлены `_TOTAL_ADJ_MIN=-8`/
   `_TOTAL_ADJ_MAX=24` для нормализации, живой `/admin/api/complex/{id}/
   location-score` не тронут, **РЕШЕНИЕ 2 плана** соблюдено, регрессии
   нет) — новый слой поверх `compute_complex_location_score()`:
   нормализация 0-100, группировка breakdown (transport/infra/noise/
   green/risk + informational для `bank` — **РЕШЕНИЕ 4**), append-only
   запись. Требование заказчика соблюдено: полный отказ Overpass даёт
   низкий `confidence`, НЕ пропуск ЖК (Unknown ≠ average) — единственная
   причина пропуска — нет резолвящихся координат вовсе. Живой баг найден
   ДО пуша и исправлен: строго последовательный проход был непрактично
   медленным на холодном `osm_cache` (~6 ЖК/5 мин, каскады "все 4
   зеркала недоступны" по 30-90с) — заменён на `asyncio.Semaphore(5)`-
   ограниченную конкурентность. 6 тестов (включая проверку, что
   параллельная обработка не путает `complex_id` между задачами).
   Таймер ежемесячно, 1 число, 09:15.

**Общий регресс**: полный прогон `./venv/bin/pytest tests/ -q` →
**431/431 passed** после каждого коммита. `krisha-apartments.service`
(правки `ai_text_analysis.py`) и `krisha-web.service` (правки
`bot/core/location_score.py`) перезапущены, git_hash проверен по
чеклисту [`process.md`](process.md#deploy-чеклист-код-на-диске--код-в-процессе).

**Гейт L1**:
- ✅ Кэш OSM реально снижает запросы к Overpass — `osm_cache` (60д TTL)
  переиспользован, не задублирован; healthcheck подтверждает состояние
  зеркал отдельно от самого запроса данных.
- ✅ Агрегаты рынка посчитаны, `resolved_house_id` учтён (complex_id-
  агрегаты) / намеренно не участвует (hex-агрегаты, обосновано выше).
- ✅ Нет регрессии в существующем `location-score` API — не менялся,
  end-to-end тесты (`test_house_resolution_geo.py`) зелёные без правок.
- ✅ Полный прогон тестов зелёный (431/431).
- ✅ У всех новых таблиц `computed_at`/`fetched_at` обязателен с первого дня.
- ⏳ **`complex_location_scores` заполнена для всех ЖК с координатами —
  ЧАСТИЧНО, backfill идёт в фоне на момент этой записи.** Реальный
  масштаб — **2079** complexes в скоупе (`is_garbage`/`is_street`
  исключены; предварительная оценка задачи была ~2305/2387 — уточнено
  по факту, не подогнано под догадку). Публичные зеркала Overpass
  сильно нестабильны на сервере (обычно жив 1-2 из 4, частые каскады
  таймаутов) — даже с ограниченной конкурентностью полный первый
  прогон занимает часы, не минуты (последующие месячные прогоны будут
  ощутимо быстрее — `osm_cache` живёт 60 дней, большая часть города уже
  будет тёплой). Прогон запущен вручную в фоне отдельно от таймера
  (следующий по расписанию — 2026-09-01), продолжает работать
  независимо от этой сессии. Первый полный охват — как только прогон
  завершится (см. `complex_location_score_snapshot.log`), гейт станет
  ✅ без дополнительного кода — это вопрос времени выполнения уже
  готового и протестированного механизма, не открытая задача.

**Дальше**: Фаза L2 (UI карточки ЖК с локационным блоком) — не начата,
следующий заход по продуктовому треку «Локация» либо по алертам (§7
`verdict_strategy.md`). Фаза C по-прежнему не начинается (пауза по ML).

---

## Часть 10. Продуктовый трек «Локация», Фаза L2 (UI) — ✅ ЗАВЕРШЕНА 2026-08-14

Закрывает Фазу L2 из [`location_product_design.md` §7](location_product_design.md#7-очередь-работ)
— план (что добавляется, откуда данные, структура блока, что НЕ
меняется, какие новые эндпойнты) показан и подтверждён заказчиком
(включая явное «НЕ дублировать turnover-dynamics/price-dynamics»,
поменявшее структуру п.3) до реализации.

1. **Бэкенд** ([`bot/core/complex_location_detail.py`](../bot/core/complex_location_detail.py) +
   `/admin/api/complex/{id}/location-detail`) — ДОПОЛНЯЕТ живой
   `/admin/api/complex/{id}/location-score` (не менялся ни строкой):
   последний снимок `complex_location_scores` (has_score=False честно,
   если backfill L1 не дошёл), плотность по гексагону+кольцо
   (`hex_market_stats`), снос в радиусе 1км (`demolition_houses`),
   POI/школы через уже кэш-aware `fetch_poi()`/новую `fetch_schools_
   poi()` (без нового живого Overpass-запроса специально для карты),
   тренд `price_drop_share_30d`. Малые аддитивные правки для координат
   точек: `bot/score_layers/poi.py` (`fetch_poi()` теперь отдаёт
   lat/lon, обратная совместимость), `bot/score_layers/schools.py`
   (новая `fetch_schools_poi()`, параллельна `compute()`). 10 тестов.
2. **Скор-карта + риск-бейджи** (`#cx-location-block` на `/complex/{id}`,
   между картой-точкой и «Что рядом») — 5 групп ровно как в `_GROUPS`
   писателя Фазы L1, риск-бейджи только при реальных основаниях (снос,
   доля снижений цены ≥20% — порог по факту распределения на живой
   выборке L1). «Расчёт ещё не выполнен», не фейковый скор, когда строки
   ещё нет. 3 теста (рендер при `geo`, отсутствие без `geo`,
   неизменность существующей карточки).
3. **Карта со слоями** (`cxlBuildMapLayers()`) — переиспользует
   существующий Leaflet instance (`window.cxLocMap`, не второй), 3
   тумблера (POI вкл. по умолчанию, снос и плотность выкл.). Плотность-
   рост как отдельный риск-бейдж НЕ добавлена (нужно ≥2 дня истории
   `hex_market_stats`, которых физически ещё нет — честно отложено, не
   реализовано ради недоступных данных). 1 новый тест.
4. **Таймлайн `price_drop_share_30d`** (`cxlRenderTrend()`) —
   hand-rolled SVG, тот же паттерн, что уже есть на странице
   (`cxdRender()`, price-dynamics) — не копия, узкая параллельная
   реализация под один метрик. Показывается только при ≥2 точках (те же
   принципы избегания фиктивной линии, что уже применены в `cxdRender()`).
   2 новых теста.

**Явно НЕ дублирует** — подтверждено заказчиком до старта: `price-
dynamics`/`turnover-dynamics` графики не тронуты, DOM/цена не
показываются повторно внутри нового блока.

**Общий регресс**: полный прогон `./venv/bin/pytest tests/ -q` →
**446/446 passed** после каждого коммита. `krisha-apartments.service`/
`krisha-web.service` перезапущены после коммита 1 (правки `bot/score_
layers/{poi,schools}.py`/`terminal_extras.py`), git_hash проверен;
Jinja-шаблоны (коммиты 2-4) авто-подхватываются без рестарта. Ручная
проверка headless-браузером недоступна в этой сессии — регрессия для
JS-разметки закрыта через `curl` на реальном ЖК (id=118) + статическую
проверку сбалансированности скобок отрендеренного вывода, не выполнением
в браузере — честно отмечено как ограничение проверки, не как «визуально
подтверждено».

**Дальше**: Фаза L3 (2GIS/воздух/walkability, каждый источник со своим
гейтом покрытия) или продуктовый трек «Персональные алерты» — на
усмотрение заказчика. Фаза C по-прежнему не начинается (пауза по ML,
[`verdict_strategy.md`](verdict_strategy.md#пауза-по-ml-2026-08-14--2026-09-14)).

---

## Часть 11. Реестр КЖК (developers.kz) — ✅ ЗАВЕРШЕНА 2026-08-15

Первый конкретный артефакт трека [`strategic_independence.md`](strategic_independence.md)
(§3.3 «данные без Крыши — юр. риски») и раздела [`liquidity_model_design.md` §7](liquidity_model_design.md#7-юридические-риски-первички).
Схема задачи изначально предполагала проектный уровень (адрес объекта,
% продаж, сроки сдачи, номер гарантии) — проверено по реальному сайту
до реализации (`curl`, не на веру): на источнике этих полей нет вообще,
данные только на уровне застройщика (БИН/юрлицо/бренд/схема гарантии/
чёрный список/список городов и ЖК). Пересмотренная схема показана и
явно одобрена заказчиком до старта, порядок коммитов подтверждён.

1. **Миграция 074** (`migrations/074_kzk_registry.sql`) — таблица
   `kzk_registry` (UNIQUE по `bin`, FK `developer_id → developers.id
   ON DELETE SET NULL`), `developers.bin` (для точного матчинга).
   5 тестов (`tests/test_kzk_registry_schema.py`).
2. **Сбор** (`kzk_registry_collect.py` + `krisha-kzk-registry.timer`,
   еженедельно — источник обновлялся раз в 2.5 недели на момент
   проверки) — парсит встроенный JSON страницы, upsert по `bin`,
   «пропавшие» БИН только логируются (`removed_bins`), не удаляются —
   решение об удалении оставлено человеку. `is_blacklisted` — сырой
   флаг источника как есть (реализация красного флага из
   `liquidity_model_design.md` §7). 8 тестов
   (`tests/test_kzk_registry_collect.py`).
3. **Матчинг** (`kzk_registry_match.py`) — двухуровневый: точный по
   БИН, иначе SQL-side batch fuzzy (`pg_trgm`, один запрос на строку
   вместо наивных ~161k пар) с теми же порогами, что и в ER-калибровке
   Фазы B п.4 (`AUTO_MATCH_THRESHOLD=0.8`/`REVIEW_QUEUE_THRESHOLD=0.5`
   из `bot/core/entity_resolution.py`). Проверено на живых данных (313
   реестр × 514 застройщиков) — ловит настоящие false positive
   («Otau Group» vs «ULYTAU GROUP») и корректно уводит их в ревью, не
   auto-match. Матчинг вручную по запросу, без таймера. 9 тестов
   (`tests/test_kzk_registry_match.py`).
4. **Админ-страница** `/admin/kzk-registry` (`bot/core/kzk_registry_
   admin.py` + `bot/templates/kzk_registry.html`) — сводка + фильтруемый
   список + confirm/reject review-кандидатов + ручной матч. Отдельная
   страница, не вкладка `/admin/parsers` (слишком много интеграционной
   поверхности для одной новой сущности) — вместо этого лёгкая видимая
   ссылка на `parsers.html`. 14 тестов
   (`tests/test_kzk_registry_admin.py`).
5. **Документация** — статус в [`strategic_independence.md` §7](strategic_independence.md#7-связь-с-текущей-работой)
   (🔴 не начато → 🟡 источник подключён, БВУ/МИО ещё нет) и в
   [`liquidity_model_design.md` §7](liquidity_model_design.md#7-юридические-риски-первички)
   (реализовано частично — `warranty_scheme` пока сырая строка, не
   разложена на три булевых БВУ/КЖК/МИО признака).

**Известное ограничение**: `reject_match()` не имеет памяти отклонения
(в отличие от `complex_source_link_rejections` в `entity_resolution.py`)
— приемлемо, т.к. матчинг не автоматизирован таймером; стоит добавить,
если матчинг когда-нибудь станет периодическим.

**Общий регресс**: `./venv/bin/pytest tests/ -q` → **489/489 passed**.

**Параллельно, отдельным треком**: интеграция школ/садиков как
факторов в `location_score.py` — план с открытыми вопросами показан
заказчику, реализация не начата (ждёт подтверждения).

---

## Часть 12. БВУ/КЖК/МИО в карточках ЖК — ✅ ЗАВЕРШЕНА 2026-08-15

Реализует §3.3/§7 [`strategic_independence.md`](strategic_independence.md)
и §7 [`liquidity_model_design.md`](liquidity_model_design.md#7-юридические-риски-первички) —
видимая пользователю юридическая защита дольщика на базе уже собранного
`kzk_registry` (Часть 11).

Три несостыковки со спекой найдены проверкой по реальным данным ДО
реализации: `bank_name` не существует в источнике вообще (сырой JSON
проверен живым запросом); `warranty_scheme` — ОДНО значение на bin, не
три независимых флага БВУ/КЖК/МИО; `developer_id -> kzk_registry` не
1:1 (один бренд — разные bin/юрлица с разным scheme/blacklist, напр.
BI Group — 9 разных bin). Резолюция подтверждена заказчиком до
реализации.

1. **`bot/core/complex_detail.py::get_kzk_info()`** — 3-уровневая
   резолюция: точный BIN через `complex_tech_specs.developer_bin`
   (ручной ввод админом, 394 реальных совпадения — надёжнее и шире
   обоих fuzzy-путей, не было в исходной спеке) → точный ЖК-матч
   (`zhk_matches.complex_id`, confidence≥0.8) → developer_id fallback
   (только подтверждённые матчи, worst-case blacklist / unanimous
   scheme). 11 тестов.
2. **UI**: `#cx-kzk-block` на `/complex/{id}` (только новостройки,
   после карточки "Описание") + риск-бейдж в модалке объявления
   (`bot/core/listing_detail.py`, только `market_type='primary'`).
   19 тестов.
3. **Документация**: `strategic_independence.md` §3.3/§6/§7 и
   `liquidity_model_design.md` §7 — статус 🟡→✅ интегрировано.

**Общий регресс**: `./venv/bin/pytest tests/ -q` → **530/530 passed**.

---

## Часть 13. Location Reliability Phase — ✅ ЗАВЕРШЕНА 2026-08-15

Внеплановый трек, инициированный заказчиком по факту живой находки при
Части 9-10-11-12 (интеграция школ/садиков, 2026-08-15): добавление
`school_access`/`kindergarten_access` расширило `_TOTAL_ADJ_MAX` с 24
до 30, из-за чего нормализованный `location_score` ВСЕХ уже посчитанных
ЖК просел (66→55 в регресс-тесте) без единого содержательного изменения
для конкретных ЖК — концептуальная ошибка модели, не баг реализации.
4 коммита, "показать план перед реализацией" не запрашивался — задача
пришла уже с готовым детальным ТЗ по всем 5 пунктам.

1. **Семантика + групповая модель** — заменил линейный `_TOTAL_ADJ_MIN/
   MAX` на взвешенное среднее по группам (`normalize_group_weighted()`):
   transport 25% / infra 25% / green 20% / noise 15% / risk 15%, веса
   структурные, не зависят от числа факторов внутри группы. Гейт на
   1749 реальных ЖК: 1577/1749 изменили score, |Δ| median=3 max=18,
   знак смешанный (не перекос).
2. **Двойные школы + building_age** — `bot/score_layers/schools.py`
   получил `university_only` (дефолт False, старое поведение для
   остальных потребителей не тронуто) — при доступных точных astana_
   schools/kindergartens данных OSM считает ТОЛЬКО вузы, не дублирует
   школьно-садиковый сигнал. `building_age` убран из location score и
   `_GROUPS` (качество здания — не локации; функция сохранена для
   будущего property_score). Гейт: 1625/1749 изменили, знак чисто
   положительный (устранение искусственного штрафа за возраст здания).
3. **Confidence redesign** — диапазон группы стал availability-aware
   (`_group_range_available()`) вместо статического: неизмеренный
   фактор исключается из диапазона ЦЕЛИКОМ (группа без сигнала = честные
   50%, не 0%) — закрывает оба известных ограничения из коммитов 1-2.
   `_compute_confidence()` взвешен по `_SOURCE_QUALITY` (точный реестр
   0.8/OSM 0.6/эвристика 0.2), не по плоской доле измеренного. Гейт:
   score сдвинулся чисто положительно (1617/1749), confidence — чисто
   отрицательно (1749/1749, честно — живой снимок недоукомплектован
   высококачественными источниками).
4. **Stability-тест** — `tests/test_location_score_stability.py`:
   добавление фактора ВНУТРИ существующей группы строго нейтрально
   (Δ=0, подтверждено на всех 1749 реальных ЖК) для тех, у кого он
   unknown. Честная находка: добавление ЦЕЛОЙ НОВОЙ группы с
   перераспределённым весом это свойство НЕ наследует автоматически
   (математически неизбежный сдвиг, если группа-донор веса была у
   своего края) — задокументировано как разница между "добавить
   фактор в группу" (безопасно всегда) и "добавить группу" (уже
   требует ручного решения по весам, см. докстринг `_GROUP_WEIGHTS`).

**Общий регресс**: `./venv/bin/pytest tests/ -q` → **557/557 passed**
(после коммита 4).

**Дальше**: Задача 3 (воздух в location_score, `_GROUPS['risks']`) —
разблокирована, эта фаза была явным условием для её начала.

---

## Часть 14. Воздух в location_score — ✅ ЗАВЕРШЕНА 2026-08-15

Последняя из трёх исходных задач захода 2026-08-15, начата только после
явного завершения Части 13 (Location Reliability Phase), как требовал
заказчик. `bot/core/location_score.py::_air_quality_factor()` — индекс
загрязнения `air_stations` (ПНЗ Казгидромет, ecodata.kz) в `_GROUPS
['risk']` (спека заказчика говорила `_GROUPS['risks']` — прочитано как
ссылка на уже существующий ключ `risk`, не запрос на новую группу).

**Проверено перед реализацией, не на веру**: `index_value` =
max(факт/ПДК м.р.) по загрязнителям (см. докстринг `pnz_collect.py`),
т.е. 1.0 — ровно на пределе допустимой концентрации. На 295 строках
истории (10 станций, не 11 как в старой заметке — пересчитано) индекс
НИ РАЗУ не превысил 0.2 — пороги заказчика (<1.0→0/1.0-2.0→-1/
2.0-5.0→-2/≥5.0→-3) реализованы как есть, но на ЖИВЫХ данных прямо
сейчас фактор даёт adj=0 для всех ЖК (честно, не занижено искусственно
под текущие цифры) — станет содержательным при реальном ухудшении
воздуха без единой правки кода. Живая проверка на 30 реальных ЖК
подтвердила: adj=0 везде, расстояния до станций 743-4461м, 4/30 вне
радиуса 5км ("нет станции в радиусе").

`air_stations` — time series (не одна строка на станцию) — берётся
самое свежее измерение КАЖДОЙ станции (`DISTINCT ON`), потом ищется
ближайшая, не наоборот (иначе риск вернуть устаревшую запись близкой
станции).

`_TOTAL_ADJ_MIN`, упомянутый в исходной спеке задачи ("-11, было -8"),
в кодовой базе больше не существует — убран Частью 13 (группы вместо
единого линейного диапазона). Air_quality интегрирован в новую
групповую модель напрямую (диапазон risk-группы -2..0 -> -5..0).

7 тестов (`tests/test_location_score_air_quality.py`): все 4 порога,
`>5км` с честным reason (не "нет данных" — тот же принцип, что "школа
дальше 1км"), выбор СВЕЖЕЙ записи ближайшей станции (не устаревшей).
`_group_range` в `tests/test_location_score_group_weighted.py`
пересчитан под новый risk-диапазон.

Документация: `location_product_design.md` §3/§4/§7 и
`strategic_independence.md` §3.3/§7 — статус 🟡→✅ интегрировано,
устаревшая заметка про "6-й источник"/IQAir fallback убрана (не
понадобился — своих данных хватило).

**Общий регресс**: `./venv/bin/pytest tests/ -q` → **571/571 passed**.

**Параллельно, отдельным треком**: другая сессия одновременно
реализует walkability (OSRM, миграция 075) в том же `location_score.py`
— их работа не коммитилась этим заходом (git apply --cached точечно на
свои хунки, включая случаи прямого построчного пересечения внутри
`compute_complex_location_score()`/`_schools_factor()` — разрешено
восстановлением из HEAD + ручным применением своего диффа, не
редактированием живого файла напрямую).

---

## Часть 15. Location Reliability Phase v2 — ✅ ЗАВЕРШЕНА 2026-08-15

Заказчик уточнил модель Части 13 сразу после её завершения: НЕ "сумма
доступных факторов", а измерительная система ПЯТИ фундаментальных
latent-свойств локации (философия — как хороший индекс инфляции:
новый товар в корзине не меняет инфляцию задним числом). 5 коммитов;
"Блок A" (БВУ/КЖК/МИО, Часть 12) и пп.3/5 ("двойные школы +
building_age", Часть 13 коммит 2) — уже полностью удовлетворяли
новым правилам, доп. работы не потребовалось, зафиксировано аудитом.

1. **Семантика + якоря + иерархическая модель** — переименовал пять
   групп в пять latent-свойств: `transport`=Accessibility 25%,
   `infra`=Everyday infrastructure 25% (обе без изменений),
   `environment`=Environment 20% (green 20%+noise 15% ОБЪЕДИНЕНЫ, было
   35% суммарно; air_quality переехал сюда из risk), `risk`=Location
   risk 15% (air_quality убран), `urban_quality`=Urban quality/
   desirability 15% — НОВОЕ, СЕЙЧАС ПУСТО, confidence=0 структурно.
   Явные якоря 0/50/100 на каждое свойство. Честное следствие пустого
   urban_quality: теоретические границы score теперь 8/92, не 0/100 —
   акт честности, не баг. SQL-колонки `noise_score`/`green_score` в
   `complex_location_scores` СОХРАНЕНЫ (без миграции). Гейт на 1749
   реальных ЖК: 1246/1749 изменили score, |Δ| median=7 max=8, знак
   смешанный (сдвиг к центру).
2. **Per-group confidence** — `_group_confidence()`: пара "score
   X/100, confidence Y%" на КАЖДОЕ свойство (не только общий confidence
   локации) — `compute_complex_location_score()["group_scores"]`/
   `["group_confidence"]` + `breakdown["_group_scores"]` в JSONB
   снимка. `urban_quality` даёт confidence=0 всегда (структурно, не
   50% как pct).
3. **Stability-тест под v2** — расширен на реальную таксономию (не
   гипотетическую), гейт на 1749 ЖК встроен прямо в pytest (не только
   разовый ручной прогон): максимальный |Δ| = 0. Зафиксировано ключевое
   различие: SCORE обязан Δ=0 при добавлении неизмеренного фактора,
   CONFIDENCE — не обязан, может честно снижаться (растёт измеримая
   вселенная) — та же логика, что в `deal_score.py`.
4. **Calibration Roadmap** — только документ
   (`location_product_design.md` §6, новый подраздел): hedonic-
   регрессия `Price = Apartment+Building+Complex+Location+Time` по
   накопленной истории цен (когда её накопится достаточно) +
   pairwise-предпочтения пользователей → Bradley-Terry/conjoint → веса
   из реальных предпочтений покупателей, в перспективе — сегментные
   скоры (семья с детьми/инвестор/без авто). Явно НЕ реализуется
   сейчас — ни истории, ни механизма сбора предпочтений пока нет.

**Общий регресс**: `./venv/bin/pytest tests/ -q` → **586/586 passed**
(после коммита 4; коммит 5 — только документация, код не менялся).

**Параллельная сессия** всё это время продолжала (уже более активно)
работу над walkability (OSRM) в тех же файлах — местами буквально в
тех же строках (`compute_complex_location_score()`'s gather()). Их
работа НЕ коммитилась ни разу — staged только свои хунки через
восстановление из HEAD + git apply --cached, при необходимости
дополнительно материализуя патч в рабочее дерево (`git apply` без
`--cached`) для честного прогона тестов перед коммитом.
