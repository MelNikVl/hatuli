...
def test_exception_in_one_complex_rolls_back_not_commits():
    """Если тело итерации бросит исключение (задача 2026-08-17, "ложный
    sentinel": find_geo_id/fetch_reviews теперь ДЕЙСТВИТЕЛЬНО поднимают
    TransientFetchError/PermanentFetchError на сбое запроса, не глотают
    его молча — этот тест бьёт по тому же пути напрямую через фейковый
    RuntimeError, не завися от конкретного типа исключения) —
    conn.rollback(), не commit() наполовину собранных вставок, и цикл
    продолжает следующий ЖК."""
