...
stats = f"\nTotal: {len(result)} | Class: {sum(1 for r in result if not r['has_class'])} | Year: {sum(1 for r in result if not r['has_year'])} | Notes: {sum(1 for r in result if not r['has_notes'])}"
